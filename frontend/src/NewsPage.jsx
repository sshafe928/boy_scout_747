    ...
    return (
        ...
        <div className='flex flex-col my-5 gap-8 flex-wrap items-center justify-center sm:flex-row'>
            {sampleEvents.map((event, index) => {
                ...
                return (
                    <div key={event.title} className={`relative font-Tienne p-8 border-b border-gray-300 flex flex-col w-4/5 h-64 sm:w-1/3 lg:w-1/4 ${index % 2 !== 0 ? "bg-brand-accent-light" : "bg-brand-accent-warm text-white"}`}>
                        ...
                        <div className={`flex justify-center items-center absolute inset-y-0 left-0 w-full text-2xl p-5 opacity-0 transition ease-in-out duration-500 hover:opacity-100 ${index % 2 !== 0 ? "bg-brand-accent-light" : "bg-brand-accent-warm text-white"}`}>Description of news</div>
                    </div>
                )
                })}
        </div>
